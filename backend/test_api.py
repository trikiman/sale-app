...
class TestFavorites:
    ...
    def test_get_favorites_empty(self):
        ...
        response = client.get(f"/favorites/{self.TEST_USER_ID}")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_toggle_favorite_removes(self):
        ...
        client.delete(f"/favorites/{self.TEST_USER_ID}/{self.TEST_PRODUCT['product_id']}")
    ...
    def test_remove_favorite(self):
        ...
        # First add
        client.post(f"/favorites/{self.TEST_USER_ID}", json=self.TEST_PRODUCT)
        
        # Then remove
        response = client.delete(
            f"/favorites/{self.TEST_USER_ID}/{self.TEST_PRODUCT['product_id']}"
        )
        assert response.status_code == 200
        data = response.json()
