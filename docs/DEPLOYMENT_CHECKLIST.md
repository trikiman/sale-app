...
| 1.7 | Database file exists or auto-creates | Start backend → `data/salebot.db` exists | ✅ 110KB |
| 1.8 | `data/` directory exists | `ls data/` — should contain `proposals.json` | ✅ |
| 1.9 | `proposals.json` is valid JSON | `python -c "import json; json.load(open('data/proposals.json'))"` | ✅ 156 products |
...
| 2.1.2 | `GET /api/products` returns products | `curl https://vkusvillsale.vercel.app/api/products` | JSON with `products` array, `updatedAt` field | ✅ 156 products |
...
| 2.1.11 | Client log rate limiter works | Send 31 requests → last should return `{"ok": false, "throttled": true}` | | ⏭️ manual |
| 2.1.12 | `GET /api/stream` opens SSE connection | `curl -N /api/stream` — holds open, receives `keepalive` within 30s | | ⏭️ timeout in script |
| 2.1.13 | `POST /api/sync` marks products as seen | `curl -X POST /api/sync` | JSON with `success: true`, `total_products` | ✅ 156 |
...
| 2.2.6 | `DELETE /api/favorites/{user_id}/{product_id}` removes | Send DELETE | `success: true` | ⏭️ manual |
| 2.2.7 | Guest user ID (string) works for favorites | Use `guest_abc123` as user_id | No crash, works normally | ⏭️ manual |
...
| 2.3.4 | Rate limiter: 4th login attempt within 10 min → 429 | Send 4 rapid login requests | 429 error | ⏭️ needs real Chrome |
...
| 2.3.13 | `POST /api/auth/verify-pin` validates PIN | Send correct/wrong PIN | Success or "Неверный PIN" | ⏭️ needs active PIN |
| 2.3.14 | PIN lockout after 5 wrong attempts | Send 5 wrong PINs | Locked response | ⏭️ needs active PIN |
| 2.3.15 | `POST /api/auth/set-pin` creates PIN | After SMS verify, set 4-digit PIN | `success: true` | ⏭️ needs active session |
| 2.3.16 | `POST /api/auth/logout` clears session | Logout with valid user_id | `success: true`, status changes to `authenticated: false` | ⏭️ needs active session |
...
| # | Test | Command / Steps | Expected | ✅/❌ |
|---|------|-----------------|----------|:-----:|
| 2.4.1 | `GET /api/cart/items/{user_id}` returns cart | Requires authenticated user | JSON with `items`, `total_price`, `items_count` | ⏭️ needs auth |
| 2.4.2 | Unauthenticated cart request → graceful fallback | Request with no cookies user | Returns fallback with `source_unavailable: true` or 401 | ⏭️ needs auth |
| 2.4.3 | `POST /api/cart/add` adds product | Send `user_id`, `product_id`, `is_green`, `price_type` | `success: true`, `cart_items` count | ⏭️ needs auth |
| 2.4.4 | `POST /api/cart/remove` removes product | Send `user_id`, `product_id` | Success response | ⏭️ needs auth |
| 2.4.5 | `POST /api/cart/clear` clears all items | Send `user_id` | Success, cart count → 0 | ⏭️ needs auth |
| 2.4.6 | IDOR protection: mismatched X-Telegram-User-Id → 403 | Cart request with wrong header user | 403 | ⏭️ needs auth |

#### 2.5 Account Linking

| # | Test | Command / Steps | Expected | ✅/❌ |
|---|------|-----------------|----------|:-----:|
| 2.5.1 | `POST /api/link/generate` creates link token | Send `guest_id: "guest_abc"` | Returns `token` + `link` URL | ⏭️ manual |
| 2.5.2 | Invalid guest_id → 400 | Send `guest_id: "invalid"` (no `guest_` prefix) | 400 error | ⏭️ manual |
| 2.5.3 | `GET /api/link/status/{guest_id}` returns status | Query unlinked guest | `linked: false` | ⏭️ manual |
| 2.5.4 | Link URL format is correct | Check returned `link` | `https://t.me/green_price_monitor_bot?start=link_...` | ⏭️ manual |
...
| 2.6.3 | Admin with valid token → accepted | Include correct `X-Admin-Token` header | 200 or scraper started | ⏭️ would trigger scraper |
| 2.6.4 | Scraper status endpoints work | `GET /api/admin/run/green/status` | JSON with `running`, `last_run`, `exit_code` | ⏭️ route requires token |
| 2.6.5 | All scraper types accessible | Check status for: `green`, `red`, `yellow`, `merge`, `categories`, `login` | Each returns valid JSON | ⏭️ route requires token |
...
| 3.3.3 | Favorite persists on refresh | Add favorite → refresh page | Heart still filled ❤️ | ⏭️ manual |
| 3.3.4 | Remove favorite works | Click ❤️ on favorited card | Returns to 🤍, persists on refresh | ⏭️ manual |
| 3.3.5 | Rapid double-click doesn't break | Click ❤️ twice quickly | No error, state stays consistent | ⏭️ manual |
| 3.3.6 | API error → rollback | Kill network → click ❤️ | Heart reverts after failed API call | ⏭️ manual |
...
| 3.4.2 | **Unauthenticated**: shows login prompt | Click cart button when not logged in | Login prompt overlay appears | ⏭️ manual |
| 3.4.3 | Login prompt "Войти" navigates to login | Click "Войти" in prompt | Login form shown | ⏭️ manual |
| 3.4.4 | Login prompt "Не сейчас" dismisses | Click "Не сейчас" | Prompt closes | ⏭️ manual |
| 3.4.5 | **Authenticated**: cart button → loading spinner | Click cart when logged in | Button shows spinner | ⏭️ needs auth |
| 3.4.6 | Success → checkmark icon | After successful add | ✓ icon for 2 seconds | ⏭️ needs auth |
| 3.4.7 | Error → X icon + toast | After failed add (sold out) | ✗ icon, toast "Этот продукт уже раскупили" | ⏭️ needs auth |
| 3.4.8 | Cart count badge updates | After add | Header 🛒 badge number increases | ⏭️ needs auth |
...
| 3.5.6 | Header title changes per filter | Solo green | Title → "🟢 Зелёные ценники" | ⏭️ manual |
| 3.5.7 | Yellow-only sorts by discount | Click only yellow | Products sorted highest discount first | ⏭️ manual |
| 3.5.8 | ❤️ favorites filter works | Click ❤️ chip | Only favorited products shown | ⏭️ manual |
...
| 3.6.3 | Click category → filters products | Click "🥬 Овощи" | Only products with category "Овощи" shown | ⏭️ manual |
| 3.6.4 | Click "Все" clears filter | Click "Все" | All products from active types shown | ⏭️ manual |
| 3.6.5 | Horizontal scroll with indicators | Scroll categories | Gradient fade indicators on left/right edges | ⏭️ manual |
| 3.6.6 | Selected chip scrolls to center | Click far-right category | Chip scrolls smoothly to center | ⏭️ manual |
| 3.6.7 | "Новинки" chip with count | If uncategorized products exist | "🆕 Новинки (N)" pinned near start | ⏭️ manual |
| 3.6.8 | Empty category → empty state message | Select category with 0 products | "В этой категории пока нет товаров" | ⏭️ manual |
...
| 3.7.4 | View mode persists on refresh | Switch to list → refresh | Still in list mode | ⏭️ manual |
...
| 3.8.3 | Theme persists on refresh | Switch to light → refresh | Still light mode | ⏭️ manual |
...
| # | Test | Steps | Expected | ✅/❌ |
|---|------|-------|----------|:-----:|
| 3.10.1 | Cart button in header opens panel | Click 🛒 in header (authenticated) | Cart panel slides up from bottom | ⏭️ needs auth |
| 3.10.2–3.10.19 | All cart panel tests | Various | Various | ⏭️ needs auth |
...
| # | Test | Steps | Expected | ✅/❌ |
|---|------|-------|----------|:-----:|
| 3.12.1–3.12.4 | All logout tests | Various | Various | ⏭️ needs active session |
...
| # | Test | Steps | Expected | ✅/❌ |
|---|------|-------|----------|:-----:|
| 3.14.1–3.14.4 | All SSE tests | Various | Various | ⏭️ manual browser |

#### 3.15 Warnings & Edge States

| # | Test | Steps | Expected | ✅/❌ |
|---|------|-------|----------|:-----:|
| 3.15.1–3.15.6 | All warning tests | Various | Various | ⏭️ manual |
...
| # | Test | Steps | Expected | ✅/❌ |
|---|------|-------|----------|:-----:|
| 4.1–4.15 | All bot commands | Various `/start`, `/help`, `/sales` etc. | Various | ⏭️ needs Telegram |
...
| 5.5 | IDOR protection on cart | Cart ops with mismatched header | 403 | ⏭️ needs auth |
| 5.6 | PIN salted hash (not plaintext) | Check `data/auth/*/pin.json` | `pin_hash` field, no raw PIN stored | ✅ |
| 5.7 | Login rate limiting | 4 rapid login attempts | 429 after 3rd for same phone | ⏭️ would trigger Chrome |
| 5.8 | Client log rate limiting | 31 rapid POST to `/api/log` | Throttled response | ⏭️ manual |
...
| # | Test | Steps | Expected | ✅/❌ |
|---|------|-------|----------|:-----:|
| 6.1–6.8 | All responsive tests | Various viewports | Various | ⏭️ manual viewport testing |
...
| 7.3 | Images load progressively | Watch product grid | Skeleton → image fade-in | ⏭️ manual |
| 7.4 | No memory leaks (SSE cleanup) | Navigate away from page | EventSource closed, intervals cleared | ⏭️ manual |
| 7.5 | Concurrent scraper lock works | Trigger same scraper twice | Second request says "Already running" | ⏭️ manual |
| 7.6 | `proposals.json` concurrent read | Heavy traffic during merge | No `JSONDecodeError` (retry logic) | ⏭️ manual |
...
| 8.4 | Auto-restart on crash | `kill -9` backend PID → wait 10s | Service auto-restarts | ⏭️ risky |
| 8.5 | Logs writing | Check `logs/backend.log` | Recent entries, no crash loops | ⏭️ manual |
...
| 8.8 | Periodic cleanup running | Check logs for cleanup messages | "Cleanup" entries every 5 min | ⏭️ manual |
...
| 9.5 | Category scraper runs | Trigger via admin API | Exit code 0, categories assigned | ⏭️ manual |
| 9.6 | Scraper lock prevents doubles | Trigger same scraper twice quickly | Second call blocked: "Already running" | ⏭️ manual |
| 9.7 | Scraper output captured | Check status endpoint after run | `last_output` shows last 40 lines | ⏭️ manual |
