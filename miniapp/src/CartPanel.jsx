...
export default function CartPanel({ isOpen, onClose, userId }) {
    ...
    const fetchCart = useCallback((showSpinner = true) => {
        ...
        setError(null)
        fetch(`/api/cart/items/${userId}`, { headers: getAuthHeaders(userId) })
            .then(res => {
            ...
            .then(data => {
                ...
                cacheRef.current = { items: newItems, totalPrice: data.total_price || 0, itemsCount: data.items_count || 0, ts: Date.now() }
            })
    ...
    const handleClearAll = async () => {
        ...
        try {
            ...
            if (res.ok) { cacheRef.current = null; fetchCart(false) }
            else { setError('Ошибка очистки') }
    ...
    return (
        <>
            <div className="cart-backdrop anim-fade" onClick={onClose} />
            <div className="cart-panel cart-panel-enter">
