"""
Red prices scraper - standalone script
Scrapes "Скоро исчезнут с полок" from VkusVill offers page.
...
async def _load_cookies(page):
    ...
    print(f"  [RED] Loaded {len(cdp_cookies)}/{len(cookies)} cookies via CDP")
...
async def scrape_red_prices_async():
    print("🔄 [RED] Starting...")
    browser = None
    proc = None
    tmp_profile = None
    ...
    try:
        ...
        page = await browser.get(RED_URL)
        ...
        """)
        print(f"  [RED] Products visible: {is_active}")
        ...
        async with scraper_slot("scrape_red"):
         ...
         for batch in range(20):
            ...
            if (in_stock_count or 0) <= prev_in_stock:
                ...
                if no_increase_count >= 2:
                    print("  [RED] No new in-stock products - done")
                    break
            else:
                no_increase_count = 0
...
def scrape_red_prices():
    if not check_vkusvill_available():
        return []
    return asyncio.run(scrape_red_prices_async())
