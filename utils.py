...
def normalize_category(raw_cat, product_name):
    ...
    if 'запеканка' in name or 'блины' in name or 'сырники' in name or 'каша' in name or 'суп' in name or 'сэндвич' in name:
        return 'Готовая еда'
    if 'мандарин' in name or 'яблок' in name or 'банан' in name or 'апельсин' in name or 'груша' in name or 'киви' in name:
        return 'Фрукты'
    if 'огурец' in name or 'томат' in name or 'помидор' in name or 'картоф' in name or 'зелень' in name:
        return 'Овощи'
    if 'йогурт' in name or 'творог' in name or 'кефир' in name or 'сметана' in name or 'молоко' in name or 'сыр' in name:
        return 'Молочка'
    if 'колбаса' in name or 'сосиски' in name or 'сардельки' in name or 'карбонад' in name or 'филе' in name or 'куриц' in name:
        return 'Мясо'
    if 'торт' in name or 'пирожн' in name or 'эклеры' in name or 'печенье' in name or 'шоколад' in name or 'конфет' in name:
        return 'Сладости'
    if 'хлеб' in name or 'батон' in name or 'лаваш' in name or 'булочка' in name:
        return 'Хлеб'
    if 'рыба' in name or 'форель' in name or 'семга' in name or 'сельдь' in name or 'креветк' in name:
        return 'Рыба'

    # Priority 2: Raw Category Mapping (if available and meaningful)
    if 'овощи' in raw_cat: return 'Овощи'
...
def deduplicate_products(products):
    """
    Removes products with duplicate IDs.
    ...
    for p in products:
        ...
        if not current_p:
            ...
            try:
                old_price = float(current_p.get('currentPrice', 0))
                new_price = float(p.get('currentPrice', 0))
